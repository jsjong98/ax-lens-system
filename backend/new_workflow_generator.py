...
## 수행 주체 (automation_level 값)
- Full-Auto: AI가 단독 수행 (사람 개입 없음)
- Human-in-Loop: AI가 수행하되 사람이 확인/승인하는 단계 포함
- Human-Supervised: 사람이 주도, AI가 보조

## 수행 주체 비율 가이드
- 전체 Task 중 **AI 단독(Full-Auto)이 60~70%** 이상이 되도록 설계하세요
- 데이터 수집, 분석, 문서 생성, 알림 발송, 패턴 탐지 등은 AI 단독 가능
- Human-in-Loop은 법적/윤리적 판단, 최종 승인 등 꼭 필요한 경우만

## Task 작성 규칙 (매우 중요)
- **task_name**: 짧고 명확하게 (예: "의료비 지급 기준 검증", "휴가 추천 발송")
  - 10~20자 이내
  - 동사형으로 끝내기 (예: "~검증", "~생성", "~발송", "~분석")
- **ai_role (설명)**: task_name과 다른 내용으로, 구체적 방법을 한 줄로 (예: "KCD 코드 매핑 후 지급 한도 자동 검증")
  - task_name을 반복하지 마세요
  - 어떤 데이터를 어떻게 처리하는지 간결하게

## 출력 규칙
- 반드시 JSON만 출력 (마크다운 코드 블록 없음)
- **기존 As-Is Task를 그대로 복사하지 말고 새로운 To-Be Task를 정의**
- Pain Point에서 영감을 받은 혁신적 Task 포함
- 에이전트는 최대 7개, 각 Task는 하나의 에이전트에 배정
- 한국어로 작성
"""
...
## 수행 주체 (automation_level 값)
- Full-Auto: AI가 단독 수행
- Human-in-Loop: AI가 수행하되 사람이 확인/승인
- Human-Supervised: 사람이 주도, AI가 보조

## 수행 주체 비율
- AI 단독(Full-Auto) 60~70% 이상
- Human-in-Loop은 법적 판단, 최종 승인 등 필수적인 경우만

## Task 작성 규칙 (반드시 지키세요)
- **task_name은 짧게** (10~20자, 동사형 종결: "~검증", "~생성", "~발송")
  - 좋은 예: "의료비 지급 기준 검증", "휴가 추천 발송", "명단 자동 확정"
  - 나쁜 예: "구성원은 누락된 성과·학습·기여 내용을 간단히 보완 입력한다" (너무 김)
- **ai_role은 task_name과 다른 내용** — 구체적 처리 방법을 한 줄로
  - 좋은 예: "KCD 코드 매핑 후 지급 한도 자동 대조"
  - 나쁜 예: task_name을 그대로 반복 (절대 금지)
- **blueprint_summary도 2~3문장으로 간결하게**
...
## 출력 형식 (JSON)
{
  ...
  "agents": [
    {
      "agent_id": "agent_1",
      "agent_name": "짧은 에이전트 이름 (예: 지급 검증기)",
      "agent_type": "에이전트 유형",
      "ai_technique": "기법 (예: LLM, RPA)",
      "description": "역할 요약 한 줄",
      "automation_level": "Full-Auto | Human-in-Loop | Human-Supervised",
      "assigned_tasks": [
        {
          "task_id": "1.1",
          "task_name": "짧은 Task명 (10~20자)",
          "l4": "상위 카테고리",
          "l3": "프로세스 영역",
          "ai_role": "task_name과 다른 구체적 처리 방법 한 줄",
          "human_role": "사람 역할 (AI 단독이면 빈 문자열)",
          "input_data": ["입력"],
          "output_data": ["출력"],
          "automation_level": "Full-Auto | Human-in-Loop | Human-Supervised"
        }
