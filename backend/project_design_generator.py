...
Step A. AI Service Flow 설계
  ...
  ※ 각 step의 세 필드는 반드시 서로 다른 내용이어야 합니다:
    • step_name  : 동사+목적어 형식의 짧은 행동 제목 (10자 이내, 예: "전략 수립", "데이터 수집")
    • description: 이 단계에서 AI/사람이 **왜** 이 행동을 하는지, **어떤 방식으로** 수행하는지를
                   1문장으로 설명 — step_name을 반복하지 말고 맥락·목적·방법을 서술
                   (예: "LLM이 업무 패턴을 분석해 최적 모니터링 키워드와 수집 주기를 자동 결정한다")
    • sub_steps  : 이 단계를 구성하는 **구체적 처리 작업** 3~5개 — description과 겹치지 않는
                   세부 액션 (예: ["RAG 기반 유사 사례 검색", "Embedding 유사도 스코어링",
                   "상위 후보 HR 담당자 전달"])
    description과 sub_steps가 같거나 유사한 내용이 되면 안 됩니다.
    ...
    "steps": [
      {
        ...
        "actor": "Senior AI",
        "description": "LLM이 업무 패턴과 과거 이력을 분석하여 최적 키워드·소스·주기를 자동 결정하고 Junior AI에게 실행 지시를 내린다",
        "sub_steps": ["과거 벤치마킹 이력 분석", "LLM 기반 핵심 키워드 도출", "소스별 신뢰도 스코어링", "수집 주기 최적화 계산"]
      },
      {
        "step_order": 2,
        "step_name": "외부 데이터 수집",
        "actor": "Junior AI",
        "description": "Senior AI의 전략 지시에 따라 외부 HRD 매체·학술 DB에서 관련 데이터를 자동으로 크롤링·정제한다",
        "sub_steps": ["웹 크롤링 실행 (ATD·SHRM 등)", "중복·저품질 데이터 필터링", "구조화 데이터로 변환·저장"]
      },
      {
        "step_order": 3,
        "step_name": "결과 검토·승인",
        "actor": "HR 담당자",
        "description": "AI가 수집·분석한 결과를 확인하고 활용 여부를 최종 판단한다",
        "sub_steps": ["AI 분석 대시보드 열람", "주요 인사이트 타당성 검토", "보고서 채택·반려 결정"]
      }
    ]
  },
  "ai_tech_info": {
