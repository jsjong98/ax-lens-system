...
def _fallback_queries(workflow_cache: dict) -> list[str]:
    """R1 fallback — L4별로 쿼리를 분산 생성 (Claude 실패 시)."""
    process_name = workflow_cache.get("process_name", "HR process")
    _, _, l4_names, _ = _extract_names_from_cache(workflow_cache)

    queries = []
    # L4마다 쿼리 1개씩 생성 (최대 6개)
    for l4 in (l4_names or [process_name])[:6]:
        en = _translate_to_en(l4)
        queries.append(
            f"global enterprise AI automation \"{en}\" case study outcome ROI 2024 2025 Fortune 500"
        )
    # 공통 보완 쿼리 4개
    focus_kr = l4_names[0] if l4_names else process_name
    focus_en = _translate_to_en(focus_kr)
    queries += [
        f"Siemens Unilever IBM GenAI \"{focus_en}\" intelligent automation enterprise results",
        f"SAP SuccessFactors Workday AI \"{focus_en}\" customer case study official 2024",
        f"삼성전자 현대자동차 포스코 '{focus_kr}' AI 자동화 도입 공식 케이스스터디 성과",
        f"McKinsey WEF Gartner HR AI transformation \"{focus_en}\" benchmark 2024 2025",
    ]
    return queries[:10]


def _fallback_r2_queries(workflow_cache: dict, r1_results: list[dict]) -> list[str]:
    """R2 fallback — R1에서 수집된 기업명 기반 심층 쿼리 생성."""
    process_name = workflow_cache.get("process_name", "HR process")
    _, _, l4_names, _ = _extract_names_from_cache(workflow_cache)
    focus_en = _translate_to_en(l4_names[1] if len(l4_names) > 1 else (l4_names[0] if l4_names else process_name))

    # R1에서 등장한 기업명 추출 (최대 3개)
    seen: list[str] = []
    for r in r1_results[:30]:
        title = r.get("title", "")
        for name in ["Google", "Microsoft", "Amazon", "IBM", "Siemens", "Unilever",
                     "P&G", "Samsung", "Walmart", "Mastercard", "Goldman"]:
            if name.lower() in title.lower() and name not in seen:
                seen.append(name)
            if len(seen) >= 3:
                break
    ...
    queries = []
    for co in seen[:3]:
        queries.append(f"{co} AI {focus_en} implementation ROI outcome official 2024 2025")

    # L4 두 번째, 세 번째 활동 커버
    for l4 in l4_names[1:4]:
        en = _translate_to_en(l4)
        queries.append(
            f"Fortune 500 enterprise GenAI LLM \"{en}\" process automation case study results"
        )
    return queries[:8]


def _fallback_r3_queries(workflow_cache: dict) -> list[str]:
    """R3 fallback — L3 전체 + 보완 관점 쿼리."""
    process_name = workflow_cache.get("process_name", "HR process")
    _, _, l4_names, _ = _extract_names_from_cache(workflow_cache)
    focus_en = _translate_to_en(process_name)

    queries = [
        f"enterprise AI transformation \"{focus_en}\" end-to-end automation 2024 2025 ROI",
        f"HR AI digital transformation \"{focus_en}\" global benchmark McKinsey Gartner 2025",
        f"agentic AI workflow \"{focus_en}\" enterprise implementation outcome official",
    ]
    # 아직 안 커버한 L4
    for l4 in l4_names[-3:]:
        en = _translate_to_en(l4)
        queries.append(f"AI \"{en}\" automation global enterprise case study 2024 cost reduction")
    return queries[:7]
...
JSON만: {{"queries": ["q1","q2","q3","q4","q5","q6","q7","q8"], "l4_focus": "어떤 L4 활동에 집중했는지 한 줄"}}"""
    ...
    try:
        ...
        print(f"[benchmark] R2 쿼리 생성 실패 → fallback 사용: {e}")
    queries = _fallback_r2_queries(workflow_cache, r1_results)
    search_log.append({"type": "plan_r2", "l4_focus": "fallback", "queries": queries, "fallback": True})
    return queries
...
JSON만: {{"queries": ["q1","q2","q3","q4","q5","q6","q7"], "gap": "R1/R2에서 부족했던 점 한 줄"}}"""
    ...
    try:
        ...
        print(f"[benchmark] R3 쿼리 생성 실패 → fallback 사용: {e}")
    queries = _fallback_r3_queries(workflow_cache)
    search_log.append({"type": "plan_r3", "gap": "fallback", "queries": queries, "fallback": True})
    return queries
